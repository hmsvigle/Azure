 ...
 * VM-A --> VM-B --> VM-C
   ...
   * Create a Route Table:
     * Address Prefix : Entire VNet Address space
     * Next Hop Type (by pass through): Virtual Appliance CIDR 
     * Next Hop (Target IP): V. Appliance Pvt IP of `VM-B`
...
* **Azure Side Steps:**
  ...
  * For P2S configuration in VN Gateway, **`Root Key`** of self-signed certificate is required. (Generated Manually & configured to Azure)
  * Save the config & Download the VPN Connection.
...
* **Client side Certificates:**
  ...
  ```
  * Extract the root certificate & configure above in VPN Gateway Configuration. 
  ```sh
  ...
  * Establish Connection
    * Extract the client certificate & place it into a Next Client Machine 
    * Install vpn config to another Client Machine to connect to Azure with P2S Connection.
...
* *Virtual Network Gateway allows to establish connectivity between two Vnets in different **`Regions`/ `Subscriptions`/ `AD Tenants`***.
...
![image](https://user-images.githubusercontent.com/24938159/196038948-a0366b49-a116-44aa-a99f-56732181dfba.png)
