...
* Un-Managed Disk: Manually manage the disk.
  * *`Ultra-Disk` is `not available` in `Centarl-US` region.*
  * For Unmanaged Disk - Always go for `Premium GPV1/GPV2` type of disk.
...
### 1.4.6 Configure Azure Disk Encryption:
  ...
  * Types of Disk Encryptions:
    * **`Server Side Encryption`:**
      * Encryption at Rest with Platform Managed Key (PMK)
      * Encryption with Customer Managed Key
      * Double Encryption with PMK & CMK
