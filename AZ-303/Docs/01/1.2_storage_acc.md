...
* Create resource -> Create Storage_Account -> `Unique Name`
  * Performance: 
    ...
    * Premium: service specic (blob/file/queue/table) --> `Low latency` & `High Performance` requirements
      * GPV2 Premium :- **Only Page Blobs** i.e `Virtual HDD`
  * Redundancy: LRS/ZRS / GRS / GZRS
  * Protection: 
    * Enable soft delete for blobs: can be restored back after certain days

 <img src="https://user-images.githubusercontent.com/24938159/123465141-5f7ddb00-d60b-11eb-8086-b3eca5d1badb.png" width="600"> 

#### * Supported Storage Service:
  
  <img src="https://user-images.githubusercontent.com/24938159/123891343-6774ac80-d976-11eb-96c0-c6b1af9719f6.png" width="700"> 

#### * Supported Storage Account Types:

  <img src="https://user-images.githubusercontent.com/24938159/123891484-98ed7800-d976-11eb-8b52-ac051311d063.png" width="700"> 


 #### Kind & Reduncandy:
  
  <img src="https://user-images.githubusercontent.com/24938159/123767775-5def3400-d8e5-11eb-953a-971c7ea4f06a.png" width="700"> 

 * **`GRS`**: It copies your data *synchronously* `three times` within a `single physical location` in the `primary` region using LRS. It then copies your data *asynchronously* to a `single physical location` in the `secondary` region. Within the secondary region, your data is copied *synchronously* three times using LRS. i.e **`6 Copies of data`**
 * **`GZRS`**: It copies your data *synchronously* across `three Availability Zones` in the primary region using ZRS. It then copies your data *asynchronously* to a single physical location in the secondary region. Within the secondary region, your data is copied synchronously three times using LRS.
...
* **Azure Storage Explorer:**
    ...
    * We can use Storage Explorer to **`Copy Data`** to Azure blob storage over internet
      * `azCopy`/`Powershell script` | `Storage Explorer` | `ADF` (Azure Data Factory) | `Azure File Sync service` | `Data Box Tools` | `Data Box gateway` | `Local files & then transfer` 
...
### 1.2.4/5/6 Authorize user/app to access services in storage account:
   ...
   * Under St Ac -> Access Keys -> show keys
     ...
     * Access Key gives `full authorization` to `all resources` (blob/file-share/tables etc) ==> To Overcome this, Use `SAS Token`
     
 #### 2. `Shared Access Signature (SAS Token)`: 
  * SAS can be generated at SA label/Blob Lavel
    * Blob-Lavel:
      * Go to SA -> container -> blob -> `Generate SAS` -> It uses key from Access-Keys section -> Update expiry date/time -> Generate
    ...
    * Account lavel:
      ...
      * Later, if we change the expiry. Then on refrshing the page in explorer, new restrictions will be auto-applied. `We can control the access`
  * Once access is given to a group. We want to control it more dynamically, to remove access before time --> Use `Stored Access Policy`
      
 #### 2.1. `Stored Access policy`:
  * To expedite expiry of SAS token before time, we can use `Stored Access Policy`. 
  * *Stored Access policy can be applied `Specifically @ container label`.*
...
### Access Tiers:
  ...
  * Archive (Option apear at blob lavel)
    ...
    * Archive Tier can be enabled at `Individual Blob` label only. 
  ...
  * The lifecycle management feature is available in all Azure regions only for **`General Purpose v2`**
 ...
 * [Storage Account Replication](https://docs.microsoft.com/en-us/azure/storage/common/storage-redundancy): 
  ...
  * **`Live Migration`**: 
    * It's supported only for St. Ac that use **`LRS`/`GRS`** replication. 
    * *If St Ac is RA-GRS --> First change replication type to either `LRS`/`GRS`, Then request Live-Migration*
 ...
 * 
 <img src="https://user-images.githubusercontent.com/24938159/122673631-ee938900-d1ee-11eb-9e69-2239c18ae3d1.png" width="700">
